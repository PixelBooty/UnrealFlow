...
namespace UnrealFlow {
  ...
  public class SyncTable {
    ...
    private string _fileLocation => Path.Combine( AppSettings.instance.settingsDirectory, this._bucket + "-sync-table.json" );
    ...
    private void _Load() {
      ...
      if( File.Exists( this._fileLocation ) ) {
        try {
          JObject loadedTable = JObject.Parse( File.ReadAllText( this._fileLocation ) );
          if( loadedTable.ContainsKey( "tree" ) ) {
            foreach( JObject file in loadedTable["tree"] as JArray ) {
    ...
    public override string ToString() {
      JObject syncTable = new JObject();
      syncTable["lastSync"] = ( new DateTime() ).ToUnixSeconds();
      syncTable["tree"] = new JArray( this._syncList.Select( x => new JObject() {
        ["path"] = x.path,
        ["lastMod"] = x.lastModified,
      } ).ToArray() );

      return syncTable.ToString();
    }
